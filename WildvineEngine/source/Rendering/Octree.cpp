...
OctreeBounds Octree::calculateRootBounds(
    ...
    // Pequeno margen para evitar errores de precision en objetos pegados a
    // los limites de la raiz.
    const float padding = (std::max)(0.01f, halfSize * 0.01f);
    halfSize += padding;
...
unsigned long long Octree::calculateSignature(
    ...
    // FNV-1a de 64 bits. Incluimos puntero, AABB y configuracion para que
    // cualquier movimiento, escala, alta/baja o cambio de Octree lo invalide.
...
void Octree::rebuild(
    ...
    const unsigned long long signature =
        calculateSignature(validEntries);
    ...
    if (!validEntries.empty()) {
        const OctreeBounds rootBounds =
            calculateRootBounds(validEntries);
